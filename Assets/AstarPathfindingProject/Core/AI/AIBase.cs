...
namespace Pathfinding
{
    ...
    public abstract class AIBase : VersionedMonoBehaviour
    {
        ...
        {
            get { return this.autoRepath.interval; }
            set { this.autoRepath.interval = value; }
        }
        ...
        {
            get { return this.autoRepath.mode != AutoRepathPolicy.Mode.Never; }
            set
            {
                if (value)
                {
        ...
        [SerializeField] [HideInInspector] [FormerlySerializedAs("centerOffset")]
        float centerOffsetCompatibility = float.NaN;

        [SerializeField] [HideInInspector] [UnityEngine.Serialization.FormerlySerializedAs("repathRate")]
        ...
        {
            get { return updatePosition ? Transform.position : simulatedPosition; }
        }
        ...
        {
            get { return updateRotation ? Transform.rotation : simulatedRotation; }
            set
            {
                if (updateRotation)
                {
        ...
        {
            get { return !waitingForPathCalculation && autoRepath.ShouldRecalculatePath((IAstarAI) this); }
        }
        ...
        public virtual void Teleport(Vector3 newPosition, bool clearPath = true)
        {
            ...
            if (updatePosition) Transform.position = newPosition;
        ...
        protected virtual void Update()
        {
            ...
            usingGravity = !(gravity == Vector3.zero) && (!updatePosition ||
                                                          ((rigid == null || rigid.isKinematic) &&
                                                           (rigid2D == null || rigid2D.isKinematic)));
        ...
        protected abstract void MovementUpdateInternal(float deltaTime, out Vector3 nextPosition,
        ...
        protected Vector3 RaycastPosition(Vector3 position, float lastElevation)
        {
            ...
            float rayLength = Transform.localScale.y * height * 0.5f + Mathf.Max(0, lastElevation - elevation);
        ...
        protected virtual void OnDrawGizmos()
        {
            ...
            {
                Draw.Gizmos.Cylinder(position, Vector3.forward, 0, radius * Transform.localScale.x, color);
            }
            else
            {
                Draw.Gizmos.Cylinder(position, rotation * Vector3.up, Transform.localScale.y * height,
                    radius * Transform.localScale.x, color);
            }
